...
leaflet(data = lex) %>% 
  ...
  addPolygons(fillColor = ~pal(rating))
```
...
# Manipulating and Generating Text

It's not the most exciting topic in the world, but it's quite likely that you'll find yourself needing to manipulate text to clean up data sets found in the real world.  Here we'll introduce a few tools that make it relatively painless to reformat or generate text in R.

## Special characters

As a reminder from our first day of class long ago, two special characters that you'll run into working with text are the tab and newline:
...
This is a very common task, particularly when you need to clean up some horrible table formatting attrocity.  You have a few different options, but the most flexible is the `gsub` function.

Here's a simple example:

```{r}
s <- "ATGCATGCATGCATGC"
gsub(pattern = "T", replacement = "U", x = s)
```

As you might guess from the variable "pattern" the search pattern that we specify can get a lot more complicated than just one letter: it can be any "regular expression."  Regular expressions are the best worst thing in the world of string searching and manipulation.  The are enormously powerful and incredibly complicated.  I'd like to recommend using a site like [RegExr](http://regexr.com/) when first playing around with regular expressions if you're trying to do something fancy.

In R see:  `?regex` for a full description of what they can do.

## Generating text

Sometimes it's easier to just generate your own new strings rather than try to clean up existing ones, if you know what pattern they should fall in.

The `rep` and `paste` functions are great for this.  Some examples
